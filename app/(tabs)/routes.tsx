...
export default function RoutesScreen() {
  // FIX: quando a tela é aberta a partir de um favorito ("Ir para Casa"/
  // "Trabalho"), esses params já trazem o destino pronto (endereço +
  // coordenadas) — ver app/(tabs)/index.tsx. Sem eles, a tela continua
  // funcionando exatamente como antes (campos em branco, preenchidos à mão).
  const params = useLocalSearchParams<{ destination?: string; destinationName?: string; destinationLat?: string; destinationLng?: string }>();
  const [origin, setOrigin] = useState("");
  const [destination, setDestination] = useState(params.destination ?? "");
  const [originPlace, setOriginPlace] = useState<AddressSuggestion | null>(null);
  const [destinationPlace, setDestinationPlace] = useState<AddressSuggestion | null>(
    params.destination && params.destinationLat && params.destinationLng
      ? { name: params.destinationName ?? params.destination, address: params.destination, latitude: Number(params.destinationLat), longitude: Number(params.destinationLng) }
      : null,
  );
  ...
  const { setActiveRoute, startTrip, setCurrentLocation } = useUrbico();
  ...
  // FIX: a lotação não tem tela própria — ela é um atributo de cada opção de
  // ônibus mostrada aqui, na busca de rota (junto do destino digitado), e
  // não só na busca por nome de linha em next-buses.tsx. Mesmo padrão de
  // consulta em lote já usado lá.
  const candidateLineIds = useMemo(() => (lineSearch.data ?? []).slice(0, 3).map((line) => line.id), [lineSearch.data]);
  const crowdBatchQuery = trpc.crowdReports.recentBatch.useQuery({ lineIds: candidateLineIds }, { enabled: candidateLineIds.length > 0, refetchInterval: 60_000 });

  // FIX: só auto-preenche a origem quando a tela foi aberta com um destino
  // pronto (veio de um favorito) — abrir /routes direto continua deixando o
  ...
  useEffect(() => {
    ...
    getCurrentUrbicoLocation()
      .then((location) => {
        ...
        setOrigin("Local atual");
        setOriginPlace({ name: "Local atual", address: "Local atual", latitude: location.latitude, longitude: location.longitude });
      })
      .catch((error) => setOriginError(error instanceof Error ? error.message : "Não foi possível obter sua localização."))
