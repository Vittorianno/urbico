...
export default function CrowdReportScreen() {
  ...
  // FIX: `addCrowdReport` só grava o relato no armazenamento local do próprio
  // aparelho (AsyncStorage) — não existe, hoje, nenhuma tabela ou endpoint no
  // servidor para relatos de lotação. O texto original prometia que o relato
  // "será combinado com relatos recentes" de outras pessoas, o que é falso no
  // estado atual do código: nada é enviado para fora do aparelho. Até que a
  // agregação colaborativa (schema + endpoint) seja implementada de fato, o
  // texto abaixo descreve o que o app realmente faz, para não prometer ao
  // usuário uma funcionalidade que não existe.
  const submit = () => { if (!selected) return; addCrowdReport(selected); Alert.alert("Relato salvo", "Seu relato foi salvo no seu histórico local. O compartilhamento com outros usuários ainda não está disponível nesta versão do Urbico.", [{ text: "Concluir", onPress: () => router.back() }]); };
  return <ScreenContainer><View style={styles.screen}><View style={styles.header}><Pressable onPress={() => router.back()} style={({ pressed }) => [styles.back, pressed && styles.pressed]}><MaterialIcons name="arrow-back" size={22} color={colors.text} /></Pressable><Text style={styles.title}>Lotação</Text><View style={styles.back} /></View><View style={styles.icon}><MaterialIcons name="groups" size={31} color={colors.cyan} /></View><Text style={styles.heading}>Como está o ônibus?</Text><Text style={styles.copy}>Por enquanto, este relato fica salvo apenas no seu aparelho, como registro pessoal. O compartilhamento anônimo com outros usuários ainda está em desenvolvimento.</Text><View style={styles.levels}>{levels.map((level) => <Pressable key={level.name} onPress={() => setSelected(level.name)} style={({ pressed }) => [styles.level, selected === level.name && { borderColor: level.color, backgroundColor: `${level.color}20` }, pressed && styles.pressed]}><View style={[styles.levelDot, { backgroundColor: level.color }]} /><View style={{ flex: 1 }}><Text style={styles.levelTitle}>{level.name}</Text><Text style={styles.levelNote}>{level.note}</Text></View><MaterialIcons name={selected === level.name ? "check-circle" : "radio-button-unchecked"} size={21} color={selected === level.name ? level.color : colors.muted} /></Pressable>)}</View><PrimaryButton label="SALVAR RELATO" icon="send" disabled={!selected} onPress={submit} style={styles.submit} /></View></ScreenContainer>;
}
